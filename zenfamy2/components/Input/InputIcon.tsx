...
const InputIcon = ({ setInput, inputvalue, label, ispassword,icon }: any) => {
    ...
    return (
        <View className="relative">
            ...
            <TextInput
                className={`w-full h-16 bg-white text-black border border-[#a5a5a53f]  rounded-full py-4 px-5 pl-14 mb-4`}
            ...
            {<TouchableOpacity onPress={() => setshowpassword((prev) => !prev)} className='absolute left-5 top-[16]'>
                {icon??<Feather name="eye-off" size={22} color="#979797" /> }
            </TouchableOpacity>}
        </View>
    );
}
